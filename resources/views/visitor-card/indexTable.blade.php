        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">Visitor Card</h1>
                    <div class="d-flex row">
                        <a href="{{ route('visitor-card.create') }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm mr-2"><i
                            class="fas fa-plus fa-sm text-white-50"></i> Create Visitor Card
                        </a>
                        <form method="GET" action="{{ route('visitor-card.batchQR') }}" >
                            ...
                            <table class="table table-bordered table-sm" id="dataTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $visitorC->visitor_code }}</td>
                                        <td>Visitor {{ $visitorC->visitor_number }}</td>
                                        <td>{{ $visitorC->status_card }}</td>
                                        <td class="text-center"><img src="{{ asset($visitorC->qr_path) }}" style="width: 60px; height: 60px;"></td>
                                         <td style="width: 8%">
                                             <center>
                                                @if (request()->get('void') == 'false' || request()->get('void') == '')
                                                <a id="show-void" class="btn btn-danger btn-circle btn-sm btn-void-record show-void" data-void-url="{{ route('visitor-card.fetchVisitorCard', $visitorC->visitor_code) }}" data-void-link="{{ route('visitor-card.void') }}" data-void-name="data-visitor-card" data-toggle="modal" data-target="#voidModal">
                                                    <i class="fas fa-ban"></i>
                                                </a>
                                                @elseif (request()->get('void') == 'true')
                                                <a id="show-restore" class="btn btn-success btn-circle btn-sm btn-restore-record show-restore" data-restore-url="{{ route('visitor-card.fetchVisitorCard', $visitorC->visitor_code) }}" data-restore-link="{{ route('visitor-card.restore') }}" data-restore-name="data-visitor-card" data-toggle="modal" data-target="#restoreModal">
                                                    <i class="fas fa-history"></i>
                                                </a>
                                                @endif

                                                <a href ="{{ route('visitor-card.generateqr', ['id' => $visitorC->visitor_code]) }}" class="btn btn-primary btn-circle btn-sm">
